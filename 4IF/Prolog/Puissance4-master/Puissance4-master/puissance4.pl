...
% Fonction qui permet d'ajouter un �l�ment en fin de liste
ajoutFin(X,[],[X]).
...
jouerCoupX(G):- write('Joueur x, entrez un num�ro de colonne : '),
				read(N), enregistrerCoup(N,G, x, X, G),
				afficherGrille(X),
				write('\n'),
				jouerCoupO(X).
jouerCoupO(G):- write('Joueur o, entrez un num�ro de colonne : '),
...
%Un coup perdant est un coup qui permet � l'adversaire de gagner
...
%Si un coup permet a l'adversaire de gagner on se d�fend(coup d�fensif).
...
%Si on a pas de coup imm�diat on fait un coup au centre ou au plus pr�s possible pour une victoire possible en verticale.
...
%Sinon jouer au plus pr�s du centre quand m�me.
...
jouerCoupJoueur(G):- write('Joueur x, entrez un num�ro de colonne : '),
