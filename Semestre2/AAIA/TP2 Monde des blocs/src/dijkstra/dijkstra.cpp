...
int main() 
{
  ...
  n0.suc.push_back( { 1, &n1 } ); n1.suc.push_back( { 3, &n2 } ); 
  n1.suc.push_back( { 1, &n3 } ); n2.suc.push_back( { 2, &n0 } ); 
  n3.suc.push_back( { 1, &n2 } ); n4.suc.push_back( { 1, &n5 } );
  n4.suc.push_back( { 3, &n6 } ); n5.suc.push_back( { 1, &n1 } ); 
  n5.suc.push_back( { 4, &n6 } );
  
  list<node*> path;
  dijkstra(&n4, &n0, path);
  
  for( node* n : path ) cout << n->val << " ; ";
  cout << endl;
  return 0;
}
